 var svgErrorIcon = '<svg style="color:red;" width="18px" height="18px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#ff0000" stroke-width="0.00024000000000000003"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M12 21C10.22 21 8.47991 20.4722 6.99987 19.4832C5.51983 18.4943 4.36628 17.0887 3.68509 15.4442C3.0039 13.7996 2.82567 11.99 3.17294 10.2442C3.5202 8.49836 4.37737 6.89472 5.63604 5.63604C6.89472 4.37737 8.49836 3.5202 10.2442 3.17294C11.99 2.82567 13.7996 3.0039 15.4442 3.68509C17.0887 4.36628 18.4943 5.51983 19.4832 6.99987C20.4722 8.47991 21 10.22 21 12C21 14.387 20.0518 16.6761 18.364 18.364C16.6761 20.0518 14.387 21 12 21ZM12 4.5C10.5166 4.5 9.0666 4.93987 7.83323 5.76398C6.59986 6.58809 5.63856 7.75943 5.07091 9.12988C4.50325 10.5003 4.35473 12.0083 4.64411 13.4632C4.9335 14.918 5.64781 16.2544 6.6967 17.3033C7.7456 18.3522 9.08197 19.0665 10.5368 19.3559C11.9917 19.6453 13.4997 19.4968 14.8701 18.9291C16.2406 18.3614 17.4119 17.4001 18.236 16.1668C19.0601 14.9334 19.5 13.4834 19.5 12C19.5 10.0109 18.7098 8.10323 17.3033 6.6967C15.8968 5.29018 13.9891 4.5 12 4.5Z" fill="#ff0000"></path><path d="M12 13C11.8019 12.9974 11.6126 12.9176 11.4725 12.7775C11.3324 12.6374 11.2526 12.4481 11.25 12.25V8.75C11.25 8.55109 11.329 8.36032 11.4697 8.21967C11.6103 8.07902 11.8011 8 12 8C12.1989 8 12.3897 8.07902 12.5303 8.21967C12.671 8.36032 12.75 8.55109 12.75 8.75V12.25C12.7474 12.4481 12.6676 12.6374 12.5275 12.7775C12.3874 12.9176 12.1981 12.9974 12 13Z" fill="#ff0000"></path><path d="M12 16C11.8019 15.9974 11.6126 15.9176 11.4725 15.7775C11.3324 15.6374 11.2526 15.4481 11.25 15.25V14.75C11.25 14.5511 11.329 14.3603 11.4697 14.2197C11.6103 14.079 11.8011 14 12 14C12.1989 14 12.3897 14.079 12.5303 14.2197C12.671 14.3603 12.75 14.5511 12.75 14.75V15.25C12.7474 15.4481 12.6676 15.6374 12.5275 15.7775C12.3874 15.9176 12.1981 15.9974 12 16Z" fill="#ff0000"></path></g></svg>';

// $(document).on('click', '.feature_request_button', function(e) {
//     console.log('123');
//     $('#feature-request-modal').addClass('show');
//     $('#feature-request-modal').find('form').trigger('reset');
// });
// $(document).on('click', '.feature_request_button_close', function(e) {
//     $('#feature-request-modal').removeClass('show');
// });
// $(document).on('click', '#submit-request-feature', function(e) {
//     e.preventDefault();
//     $('.request_feature_form').submit();
// });

// $(document).on('submit', '.request_feature_form', function(e) {
//     e.preventDefault();
//     $('#loading-bar').css('width', '0%').show();
//     animateLoadingBar();
//     $.ajax({
//         url: $(this).attr('action'),
//         type: 'POST',
//         data: new FormData(this),
//         processData: false,
//         contentType: false,
//         success: function (response) {
//             if (response.success) {
//                 $('#loading-bar').css('width', '100%');
//                 setTimeout(function() {
//                     $('#loading-bar').hide();
//                     $('#success-message').fadeIn().delay(3000).fadeOut();
//                 }, 500);
//                 setTimeout(function() {
//                     $('#feature-request-modal').removeClass('show');
//                 }, 3500);
//             }
//         },
//         error: function (xhr) {
//             $('.redError').html('');
//             $('#loading-bar').hide();
//             if (xhr.responseJSON && xhr.responseJSON.errors) {
//                 $.each(xhr.responseJSON.errors, function(field, messages) {
//                     if (field.includes('.')) {
//                         var baseField = field.split('.')[0];
//                         $('#' + baseField + '_error').html(messages[0]).prepend(svgErrorIcon).show();
//                     } else {
//                         $('#' + field + '_error').html(messages[0]).prepend(svgErrorIcon).show();
//                     }
//                     setTimeout(function() {
//                         if (field.includes('.')) {
//                             $('#' + baseField + '_error').html('');
//                         } else {
//                             $('#' + field + '_error').html('');
//                         }
//                     }, 4000);
//                 });
//             } else {
//                 console.log(xhr.responseText);
//             }
//         }
//     });
// });

// function animateLoadingBar() {
//     let width = 0;
//     const interval = setInterval(function() {
//         if (width >= 90) {
//             clearInterval(interval);
//         } else {
//             width += 10;
//             $('#loading-bar').css('width', width + '%');
//         }
//     }, 200);
// }


$(document).on('click', '.feature_request_button', function (e) {
    e.preventDefault();
    
    // Check if Bootstrap is handling the modal
    if ($.fn.modal) {
        $('#feature-request-modal').modal('show'); // Bootstrap method
    } else {
        $('#feature-request-modal').addClass('show').show(); // Fallback
    }

    $('#feature-request-modal').find('form').trigger('reset');
});

// $(document).on('click', '.feature_request_button_close', function (e) {
//     e.preventDefault();

//     // Ensure modal is selected correctly
//     var modal = $('#feature-request-modal');

//     if ($.fn.modal && modal.length) {
//         modal.modal('hide');
//     } else {
//         modal.removeClass('show').hide();
//         $('.modal-backdrop').remove(); // Remove the backdrop if it remains
//         $('body').removeClass('modal-open'); // Prevent body scrolling issue
//     }
// });

// $(document).on('click', '#submit-request-feature', function (e) {
//     e.preventDefault();
//     $('.request_feature_form').submit();
// });

// $(document).on('submit', '.request_feature_form', function (e) {
//     e.preventDefault();
//     $('#loading-bar').css('width', '0%').show();
//     animateLoadingBar();

//     $.ajax({
//         url: $(this).attr('action'),
//         type: 'POST',
//         data: new FormData(this),
//         processData: false,
//         contentType: false,
//         success: function (response) {
//             if (response.success) {
//                 $('#loading-bar').css('width', '100%');
//                 setTimeout(function () {
//                     $('#loading-bar').hide();
//                     $('#success-message').fadeIn().delay(3000).fadeOut();
//                 }, 500);
//                 setTimeout(function () {
//                     if ($.fn.modal) {
//                         $('#feature-request-modal').modal('hide');
//                     } else {
//                         $('#feature-request-modal').removeClass('show').hide();
//                     }
//                 }, 3500);
//             }
//         },
//         error: function (xhr) {
//             $('.redError').html('');
//             $('#loading-bar').hide();
//             if (xhr.responseJSON && xhr.responseJSON.errors) {
//                 $.each(xhr.responseJSON.errors, function (field, messages) {
//                     var baseField = field.includes('.') ? field.split('.')[0] : field;
//                     $('#' + baseField + '_error').html(messages[0]).prepend(svgErrorIcon).show();
                    
//                     setTimeout(function () {
//                         $('#' + baseField + '_error').html('');
//                     }, 4000);
//                 });
//             } else {
//                 console.log(xhr.responseText);
//             }
//         }
//     });
// });


// $(document).on('click', '.feature_request_button_close', function (e) {
//     e.preventDefault();

//     var modal = $('#feature-request-modal');

//     if ($.fn.modal && modal.length) {
//         modal.modal('hide');
//     } else {
//         modal.removeClass('show').hide();
//         $('.modal-backdrop').remove();
//         $('body').removeClass('modal-open');
//     }
// });

// $(document).on('click', '#submit-request-feature', function (e) {
//     e.preventDefault();
//     $('.request_feature_form').submit();
// });

// $(document).on('submit', '.request_feature_form', function (e) {
//     e.preventDefault();
//     $('#loading-bar').css('width', '0%').show();
//     animateLoadingBar();

//     $.ajax({
//         url: $(this).attr('action'),
//         type: 'POST',
//         data: new FormData(this),
//         processData: false,
//         contentType: false,
//         success: function (response) {
//             if (response.success) {
//                 $('#loading-bar').css('width', '100%');
//                 setTimeout(function () {
//                     $('#loading-bar').hide();
//                     $('#success-message').fadeIn().delay(3000).fadeOut();
//                 }, 500);
//                 setTimeout(function () {
//                     if ($.fn.modal) {
//                         $('#feature-request-modal').modal('hide');
//                     } else {
//                         $('#feature-request-modal').removeClass('show').hide();
//                     }
//                 }, 3500);
//             }
//         },
//         error: function (xhr) {
//             $('.redError').html('');
//             $('#loading-bar').hide();
//             if (xhr.responseJSON && xhr.responseJSON.errors) {
//                 $.each(xhr.responseJSON.errors, function (field, messages) {
//                     var baseField = field.includes('.') ? field.split('.')[0] : field;
//                     $('#' + baseField + '_error').html(messages[0]).prepend(svgErrorIcon).show();
                    
//                     setTimeout(function () {
//                         $('#' + baseField + '_error').html('');
//                     }, 4000);
//                 });
//             } else {
//                 console.log(xhr.responseText);
//             }
//         }
//     });
// });


$(document).on('click', '.feature_request_button_close', function (e) {
    e.preventDefault();

    var modal = $('#feature-request-modal');

    if ($.fn.modal && modal.length) {
        modal.modal('hide');
    } else {
        modal.removeClass('show').hide();
        $('.modal-backdrop').remove();
        $('body').removeClass('modal-open');
    }
});

$(document).on('click', '#submit-request-feature', function (e) {
    e.preventDefault();
    $('.request_feature_form').submit();
});

$(document).on('submit', '.request_feature_form', function (e) {
    e.preventDefault();
    $('#loading-bar').css('width', '0%').show();
    animateLoadingBar();

    $.ajax({
        url: $(this).attr('action'),
        type: 'POST',
        data: new FormData(this),
        processData: false,
        contentType: false,
        success: function (response) {
            if (response.success) {
                $('#loading-bar').css('width', '100%');
                setTimeout(function () {
                    $('#loading-bar').hide();
                    $('#success-message').fadeIn().delay(3000).fadeOut();
                }, 500);
                setTimeout(function () {
                    if ($.fn.modal) {
                        $('#feature-request-modal').modal('hide');
                    } else {
                        $('#feature-request-modal').removeClass('show').hide();
                    }
                }, 3500);
            }
        },
        error: function (xhr) {
            $('.redError').html('');
            $('#loading-bar').hide();
            if (xhr.responseJSON && xhr.responseJSON.errors) {
                $.each(xhr.responseJSON.errors, function (field, messages) {
                    var baseField = field.includes('.') ? field.split('.')[0] : field;
                    $('#' + baseField + '_error').html(messages[0]).prepend(svgErrorIcon).show();
                    
                    setTimeout(function () {
                        $('#' + baseField + '_error').html('');
                    }, 4000);
                });
            } else {
                console.log(xhr.responseText);
            }
        }
    });
});


function animateLoadingBar() {
    let width = 0;
    const interval = setInterval(function () {
        if (width >= 90) {
            clearInterval(interval);
        } else {
            width += 10;
            $('#loading-bar').css('width', width + '%');
        }
    }, 200);
}



